...
describe("template exports", () => {
  it("uses the required org/template/date filename convention", () => {
    expect(
      buildExportFileName({
        ...
        templateName: "Board Self-Evaluation",
        format: "pdf",
        date: new Date("2026-06-19T12:00:00Z"),
      }),
    ).toBe("Olea-QA-Foundation_Board-Self-Evaluation_2026-06-19.pdf");
  });
  ...
  it("renders PDF and DOCX buffers for long repeatable bilingual content", async () => {
    ...
    expect(pdf.metadata).toMatchObject({
      ...
      creator: brandName,
      producer: brandName,
      subject: `Board-ready document generated by ${brandName}`,
      title: "Olea QA Foundation Board Self-Evaluation",
    });
    expect(pdf.text).toContain("Board Self-Evaluation");
    expect(pdf.text).toContain("Olea QA Foundation");
    expect(pdf.text.replace(/\s+/g, "")).toContain("BOARD-READYREPORT");
    expect(pdf.text).toContain(`Generated by ${brandName}`);
