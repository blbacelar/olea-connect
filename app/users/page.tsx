...
const copy: Record<Locale, {
  ...
  "en-CA": {
    ...
    memberDescription: "Explore accounts created on Olea Connects. Sponsor badges identify accounts connected to active sponsors.",
    adminDescription: "All registered accounts across Olea Connects, including private account details.",
  ...
  "fr-CA": {
    ...
    memberDescription: "Découvrez les comptes créés sur Olea Connects. Un badge indique les comptes liés à un commanditaire actif.",
    adminDescription: "Tous les comptes inscrits à Olea Connects, y compris les renseignements privés des comptes.",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-slate-600">
          {directory.canViewPrivateDetails ? text.adminDescription : text.memberDescription}
        </p>
      </div>
                ...
                {directory.users.map((user) => (
                  <TableRow key={user.id} data-testid="directory-user-row">
                    <TableCell>
                      ...
                      {directory.canViewPrivateDetails && user.sponsorNames.length > 0 ? (
            ...
            <div className="flex gap-2">
              {directory.page > 1 ? (
                <Button asChild variant="outline" size="sm">
                  <Link href={`/users?page=${directory.page - 1}`}>{text.previous}</Link>
                </Button>
              ) : null}
              {directory.page < directory.lastPage ? (
                <Button asChild variant="outline" size="sm">
                  <Link href={`/users?page=${directory.page + 1}`}>{text.next}</Link>
                </Button>
              ) : null}
            </div>
          </nav>
